  ...
  <div class="mobile-modal" :style="resolutionStyle" ref="modal">
    ...
    <div class="resolution-ratio-content">
      <div class="resolution-ratio-box">
        <div v-for="item in resolutions.list" :key="item.label" style="touch-action:auto;">
          <div :class="{
...
const props = defineProps(['resolution','newResolutionsItem']);

watch(props.newResolutionsItem, (newval) => {
  if(newval) {
    let found = false;
...
const resolutionStyle = computed(() => {
  let obj = {};
  if(commStore.screenOrientation==='landscape' || commStore.mobileForceLandScape) {
    obj = {
      'width': '34.31rem',
      'height': commStore.viewPort.height +'px',
      'right': modalStreamStore.isShowResolutionRatioModal || modalStreamStore.isShowResolutionRatioModalForSetting ? '0' : '-'+ '34.31rem'
    }
  } else {
    obj = {
      'width': '100%',
      'height': commStore.viewPort.height +'px',
      'right': modalStreamStore.isShowResolutionRatioModal ? '0' : '-'+ '100%'
    }
  }
  return obj
});
...
const closeModal = () => {
  modalStreamStore.setIsShowResolutionRatioModal(false);
  modalStreamStore.setIsShowResolutionRatioModalForSetting(false);
};
const emits = defineEmits(['sendResolution']);
const changeResolutionRatio = (val) => {
  emits('sendResolution', val);
  closeModal();
};
